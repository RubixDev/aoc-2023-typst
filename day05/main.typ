...
#let seed-ranges = range(start-seeds.len(), step: 2).map(i => {
  ...
  (..range, initial-start: range.start, stage: "seed")
})
#let finished-ranges = ()
#let prev-stage = ""
#while seed-ranges.len() > 0 {
  ...
  let next-stage = maps.at(range.stage).dest
  ...
  for map-range in maps.at(range.stage).ranges {
    ...
    if range.start >= map-range.src-start and range-end <= map-range-end {
      any-overlap = true
      // range is fully contained in map range
      // -> adjust start and advance stage
      range.start += map-range.dest-start - map-range.src-start
      range.stage = next-stage
      seed-ranges.push(range)
    } else if range.start >= map-range.src-start and range.start < map-range-end {
      any-overlap = true
      // range start is in map range, end is outside
      // -> split at map range end, adjust and advance first half, keep looking for second half
      let split-len = map-range-end - range.start
      seed-ranges.push((
        start: range.start + (map-range.dest-start - map-range.src-start),
        len: split-len,
        initial-start: range.initial-start,
        stage: next-stage,
      ))
      range.start = map-range-end
      range.len -= split-len
      range.initial-start += split-len
      seed-ranges.insert(0, range)
    } else if range-end > map-range.src-start and range-end <= map-range-end {
      any-overlap = true
      // range end is in map range, start is outside
      // -> split at map range start, adjust and advance second half, keep looking for first half
      let split-len = map-range.src-start - range.start
      seed-ranges.push((
        start: map-range.dest-start,
        len: range.len - split-len,
        initial-start: range.initial-start + split-len,
        stage: next-stage,
      ))
      range.len = split-len
      seed-ranges.insert(0, range)
    } else if range.start < map-range.src-start and range-end > map-range-end {
      any-overlap = true
      // range fully contains the map range
      // -> split at both points, adjust and advance the middle section, keep looking for the outer two
      ...
      seed-ranges.push((
        start: map-range.dest-start,
        len: split-2 - split-1,
        initial-start: range.initial-start + split-1,
        stage: next-stage,
      ))
      ...
      range.start = map-range-end
      range.len -= split-2
      range.initial-start += split-2
      seed-ranges.insert(1, range)
    }
